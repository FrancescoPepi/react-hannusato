...
const FlashListCustom = ({ data, logoMap, searchQuery, setSearchQuery, onSearch, isLoading }) => {
	// console.log("FlashListCustom2 - component render - searchQuery:", searchQuery);
	const searchQuery2 = searchQuery.length > 0 && searchQuery;
	// console.log("FlashListCustom2 - searchQuery2:", searchQuery2);
	...
	// Stato per il filtro prezzo: disabilitato di default (isPriceFilterActive = false)
	const [isPriceFilterActive, setIsPriceFilterActive] = useState(false);
	const [priceRange, setPriceRange] = useState([minPrice, maxPriceSlide]);

	// Reset del filtro prezzo quando cambia la ricerca
	useEffect(() => {
		// Disattiva il filtro prezzo all'inizio di ogni nuova ricerca
		setIsPriceFilterActive(false);
		// Resetta il range di prezzo ai valori di default
		setPriceRange([minPrice, maxPriceSlide]);
	}, [searchQuery, minPrice, maxPriceSlide]);
	...
	const filteredData = data.filter((item) => {
		const matchesTitle = isTitleFilterActive
			? searchQuery2
				? item.title.toLowerCase().includes(searchQuery2.toLowerCase())
				: true
			: true;
		const matchesSource = selectedSources.length > 0 ? selectedSources.includes(item.source.toLowerCase()) : true;
		// Applica il filtro prezzo solo se isPriceFilterActive è true
		const matchesPrice = isPriceFilterActive ? item.price >= priceRange[0] && item.price <= priceRange[1] : true;
		return matchesTitle && matchesSource && matchesPrice;
	});
	...
	// If data is loading, show skeleton animation
	if (isLoading) {
	...
	return (
		<LinearGradient colors={["#56a06f", "#002f06"]}>
			<View style={[{ minHeight: 2, width: width, height: height }]}>
				...
				<View
					className="z-20 w-full flex-row items-center px-1 gap-2"
					style={[styles.stickyHeader, { height: 50, top: Platform.OS === "ios" ? 45 : 35 }]}
				>
            ...
            /> */}
						<DropdownFilter
							...
							onTitleFilterToggle={setIsTitleFilterActive}
							minPrice={minPrice}
							maxPrice={maxPriceSlide}
							priceRange={priceRange}
							onPriceRangeChange={(range) => {
								setPriceRange(range);
								// Activate price filter when user applies a price range
					...
					ListHeaderComponent={
						...
						>
							...
							>
								...
								{/* Mostra il range di prezzo solo se il filtro prezzo è attivo */}
